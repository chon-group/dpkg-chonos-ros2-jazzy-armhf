...
example_interfaces__msg__Empty__create(void)
{
...
example_interfaces__msg__Empty__Sequence__init(example_interfaces__msg__Empty__Sequence * array, size_t size)
{
  ...
  if (size) {
    if (size > SIZE_MAX / sizeof(example_interfaces__msg__Empty)) {
      return false;
    }
  ...
  example_interfaces__msg__Empty__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    if (input->size > SIZE_MAX / sizeof(example_interfaces__msg__Empty)) {
      return false;
    }
    ...
    example_interfaces__msg__Empty * data =
      (example_interfaces__msg__Empty *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!example_interfaces__msg__Empty__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          example_interfaces__msg__Empty__fini(&output->data[i]);
        }
        return false;
      }
    }
