...
example_interfaces__msg__UInt32MultiArray__create(void)
{
...
example_interfaces__msg__UInt32MultiArray__Sequence__init(example_interfaces__msg__UInt32MultiArray__Sequence * array, size_t size)
{
  ...
  if (size) {
    if (size > SIZE_MAX / sizeof(example_interfaces__msg__UInt32MultiArray)) {
      return false;
    }
  ...
  example_interfaces__msg__UInt32MultiArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    if (input->size > SIZE_MAX / sizeof(example_interfaces__msg__UInt32MultiArray)) {
      return false;
    }
    ...
    example_interfaces__msg__UInt32MultiArray * data =
      (example_interfaces__msg__UInt32MultiArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!example_interfaces__msg__UInt32MultiArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          example_interfaces__msg__UInt32MultiArray__fini(&output->data[i]);
        }
        return false;
      }
    }
